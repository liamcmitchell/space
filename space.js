  ...
  if (system.children) {
    system.children.forEach(function(child) {
      ...
      if (!child.orbitVelocity) {
        child.orbitVelocity = getOrbitVelocity(system.mass, getTotalMass(childBodies), child.orbitRadius)        
      }
...
function getForceOn(body, bodies) {
  return bodies.reduce(function(memo, otherBody) {
    if (body === otherBody) {
      return memo
    }
...
function tick() {
  ...
  if (KEYS['38'] && !STATE.ship.destroyed) { // Up
    // Thrust
  ...
  if (STATE.ship.destroyed) {
    ...
    if (getBodyRadius(nearestBody) > getDistance(nearestBody.location, STATE.ship.location) - 1) {
...
function draw(shapes) {
  shapes.forEach(function(shape) {
    if (shape.type == 'circle') {
      ctx.beginPath()
      ctx.arc(shape.center[0], shape.center[1], shape.radius, 0, Math.PI * 2, shape.filled)
...
function drawShip(ship) {
  ...
  if (ship.destroyed) {
    ...
    shapes.push({
      ...
      strokeStyle: 'rgba(255, ' + Math.round(Math.max(0, 255 - (ship.destroyedFor * 1000))) + ', 0, ' + Math.max(0, 1 - (ship.destroyedFor * 4)) + ')'
    })
  }
  else {
    if (ship.thrusting) {
      ...
      shapes.push({
        type: 'line',
        points: [[-0.25, 0.25], [0, 1], [0.25, 0.25]],
        strokeStyle: '#ff0'
      })
    }
    // Ship body
    shapes.push({
      type: 'line',
      points: [[0, 0], [1, 1], [0, -2], [-1, 1], [0, 0]],
      strokeStyle: '#33f'
    })
  }
  return translateShapes(
    scaleShapes(
      rotateShapes(shapes, [0, 0], Math.PI + ship.angle),
      1
    ),
    ship.location
  )
...
STATE.ship = {
  ...
  location: translate(bodyToStartOn.location,
    [10, -getBodyRadius(bodyToStartOn) + 10]
  ),
